...
export default function ClientDetails({ client, loading }: ClientDetailsProps) {
  ...
  return (
    <div className="p-6">
      ...
      {/* Commissions Section */}
      <div className="mb-8">
        <h2 className="text-xl font-semibold mb-4 text-gray-300 border-b border-white/10 pb-2">Commissions</h2>
        {client.commissions.length === 0 ? (
          ...
          <div className="grid gap-4 grid-cols-1 md:grid-cols-2">
            {client.commissions.map((commission, idx) => (
              <div key={commission.id} className="glass p-4 rounded-lg shadow-lg animate-fadeIn" style={{ animationDelay: `${idx * 70}ms` }}>
                ...
                <div className="mt-3 text-sm text-gray-500">
                  <p>Price: <span className="text-gray-300">${commission.price.toFixed(2)}</span></p>
                  <p>Start Date: <span className="text-gray-300">{formatDate(commission.startDate)}</span></p>
                  <p>Due Date: <span className="text-gray-300">{formatDate(commission.dueDate)}</span></p>
                  {commission.completedAt && (
                    <p>Completed: <span className="text-gray-300">{formatDate(commission.completedAt)}</span></p>
                  )}
                </div>
              </div>
            ))}
      ...
      {/* Characters Section */}
      <div className="mb-8">
        <h2 className="text-xl font-semibold mb-4 text-gray-300 border-b border-white/10 pb-2">Characters</h2>
        {client.characters.length === 0 ? (
          ...
          <div className="grid gap-4 grid-cols-1 md:grid-cols-3">
            {client.characters.map((character, idx) => (
              <div key={character.id} className="glass p-4 rounded-lg shadow-lg animate-fadeIn" style={{ animationDelay: `${idx * 70}ms` }}>
      ...
      {/* Notes Section */}
      <div>
        <h2 className="text-xl font-semibold mb-4 text-gray-300 border-b border-white/10 pb-2">Notes</h2>
        {client.notes.length === 0 ? (
          <p className="text-gray-500">No notes found for this client.</p>
        ) : (
          <div className="space-y-3">
            {client.notes.map((note, idx) => (
              <div key={note.id} className="glass p-4 rounded-lg shadow-lg animate-fadeIn" style={{ animationDelay: `${idx * 70}ms` }}>
